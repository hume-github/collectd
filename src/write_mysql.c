...
#define TAKE_MYSQL_LOCK(x, l) \
    if (pthread_mutex_lock (&(x)->mutexdb)) \
        ERROR ("write_mysql plugin: Unable to obtain lock on database handle.  errno=%d\n", errno); \
    (x)->lock_holder	= pthread_self (); \
    assert ((x)->lock_holder == pthread_self ());
/*    DEBUG("write_mysql plugin: Thread %u gained lock on database handle.", (x)->lock_holder);	*/

#define RELEASE_MYSQL_LOCK(x, l) \
    assert ((x)->lock_holder == pthread_self ()); \
    (x)->lock_holder	= -1; \
    if (pthread_mutex_unlock (&(x)->mutexdb)) \
        ERROR ("write_mysql plugin: Error unlocking mutex for database handle.  errno=%d\n", errno);
/*    DEBUG("write_mysql plugin: Thread %u released lock on database handle.", pthread_self ());    */
...
write_mysql_config_database (oconfig_item_t *ci)
{
	...
	db = (mysql_database_t *) malloc (sizeof (mysql_database_t));
	if (db == NULL)
	{
	...
	memset (db, 0, sizeof (mysql_database_t));
	...
	pthread_mutex_init (&db->mutexdb, NULL);
	pthread_mutex_init (&db->mutexhost_tree, NULL);
	pthread_mutex_init (&db->mutexplugin_tree, NULL);
        pthread_mutex_init (&db->mutextype_tree, NULL);
	pthread_mutex_init (&db->mutexdataset_tree, NULL);
...
write_mysql_init (user_data_t * ud)
{
    ...
    /*
     *	mysql_library_init must be called before any other
     *	functions.  It must either be called before threads
     *	are spawned (which isn't possible in a collectd plugin)
     *	or protected with a mutex.
     */

    TAKE_MYSQL_LOCK(db, __LINE__);

    if (db->conn == NULL) {
	if (mysql_library_init (0, NULL, NULL)) {
	    ERROR ("write_mysql plugin: mysql_library_init() failed.");
	}

	db->conn = mysql_init (NULL);
    } else {
	if (mysql_thread_init ()) {
	    ERROR ("write_mysql plugin: mysql_thread_init() failed.");
	}
    }

    RELEASE_MYSQL_LOCK(db, __LINE__);
...
add_dataset_id (data_source_t * ds, int type_id, user_data_t * ud)
{
  ...
  if (mysql_stmt_store_result (stmt) != 0)
	...
	*id = mysql_stmt_insert_id (stmt);
	mysql_stmt_close (stmt);
	RELEASE_MYSQL_LOCK(db, __LINE__);
	DEBUG ("insert dataset_id in DB : %d (%s) (%d)", *id, ds->name, type_id);
    }
